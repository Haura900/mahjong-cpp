...
TEST_CASE("deep-search pruning flags are independent and default-off")
{
    Context context;
    const PlayerState player = player_for("347789m589p3s5677z");
    ExpectedScoreCalculator::Config baseline;
    baseline.t_max = 1;
    baseline.enable_shanten_down = true;
    baseline.enable_tegawari = true;

    const auto [baseline_stats, baseline_searched] = ExpectedScoreCalculator::calc(
        baseline, context.table_config, context.round, context.table, player);
    REQUIRE(baseline_searched == 142884);

    auto shanten_down = baseline;
    shanten_down.prune_shanten_down_with_multiple_discards = true;
    const auto [shanten_down_stats, shanten_down_searched] =
        ExpectedScoreCalculator::calc(shanten_down, context.table_config,
                                      context.round, context.table, player);
    CHECK(shanten_down_searched < baseline_searched);

    auto noop = baseline;
    noop.prune_noop_tegawari = true;
    const auto [noop_stats, noop_searched] = ExpectedScoreCalculator::calc(
        noop, context.table_config, context.round, context.table, player);
    CHECK(noop_searched <= baseline_searched);

    for (const auto *stats : {&baseline_stats, &shanten_down_stats, &noop_stats}) {
        REQUIRE_FALSE(stats->empty());
        for (const auto &stat : *stats) {
            CHECK(std::isfinite(stat.exp_score[1]));
        }
...
TEST_CASE("four-shanten pruning removes only deep-search extensions")
{
    Context context;
    const PlayerState player = player_for("147m258p369s11122z");
    ExpectedScoreCalculator::Config baseline;
    baseline.t_max = 4;
    baseline.extra = 1;
    const auto [baseline_stats, baseline_searched] = ExpectedScoreCalculator::calc(
    ...
    pruned.prune_high_shanten_deep_search = true;
    ...
    CHECK(pruned_searched < baseline_searched);
    REQUIRE_FALSE(pruned_stats.empty());
    for (const auto &stat : pruned_stats) {
        CHECK(std::any_of(baseline_stats.begin(), baseline_stats.end(),
                          [&stat](const auto &base) { return base.tile == stat.tile; }));
    }
    for (const auto &stat : pruned_stats) {
        CHECK(std::isfinite(stat.exp_score[1]));
    }
...
TEST_CASE("zero and one shanten are protected from all new pruning")
{
    Context context;
    for (const char *hand : {"222567m345p33667s", "1234m23467p12406s"}) {
        const PlayerState player = player_for(hand);
        const int shanten = std::get<1>(ShantenCalculator::calc(
            player.hand, 0, ShantenFlag::All, context.table_config.game_mode));
        REQUIRE((shanten == 0 || shanten == 1));
        ExpectedScoreCalculator::Config baseline;
        baseline.t_max = 3;
        const auto [baseline_stats, baseline_searched] =
            ExpectedScoreCalculator::calc(baseline, context.table_config,
                                          context.round, context.table, player);
        auto pruned = baseline;
        pruned.prune_high_shanten_deep_search = true;
        pruned.prune_shanten_down_with_multiple_discards = true;
        pruned.prune_noop_tegawari = true;
        const auto [pruned_stats, pruned_searched] = ExpectedScoreCalculator::calc(
            pruned, context.table_config, context.round, context.table, player);
        CHECK(pruned_searched == baseline_searched);
        REQUIRE(pruned_stats.size() == baseline_stats.size());
    }
}

TEST_CASE("noop pruning preserves red-five exchange candidates")
{
    Context context;
    const PlayerState player = player_for("055m147p258s12345z");
    ExpectedScoreCalculator::Config config;
    config.t_max = 2;
    config.prune_noop_tegawari = true;
    const auto [stats, searched] = ExpectedScoreCalculator::calc(
        config, context.table_config, context.round, context.table, player);
    REQUIRE(searched > 0);
    CHECK(std::any_of(stats.begin(), stats.end(), [](const auto &stat) {
        return stat.tile == Tile::Manzu5;
    }));
    CHECK(std::any_of(stats.begin(), stats.end(), [](const auto &stat) {
        return stat.tile == Tile::RedManzu5;
    }));
    for (const auto &stat : stats) {
        CHECK(std::all_of(stat.exp_score.begin(), stat.exp_score.end(),
