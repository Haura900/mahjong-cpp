#!/usr/bin/env python3
"""Run the five pruning configurations for the three documented 4-shanten hands."""

import argparse
import json
from pathlib import Path

from benchmark_nanikiru_pruning import CONFIGS, DEFAULT_SETTINGS, Engine, build_request


HANDS = [
    "147m258p369s11122z",
    "133m188p469s12467z",
    "2359m23358p89s125z",
]


def value_at(values, turn):
    return float(values[min(max(1, turn), len(values) - 1)]) if values else 0.0


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--engine", type=Path, required=True)
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--port", type=int, default=50127)
    args = parser.parse_args()

    engine = Engine(None, args.engine, args.port)
    results = []
    try:
        for hand in HANDS:
            problem = {"hand": hand, "settings": {"turn": 6}}
            for name, flags in CONFIGS.items():
                settings = {**DEFAULT_SETTINGS,
                            "simulator_auto_disable_deep_search": False}
                request, turn = build_request(problem, settings, flags)
                raw, wall_ms = engine.calculate(request)
                rows = []
                for stat in raw.get("stats", []):
                    tile = int(stat.get("tile", -1))
                    if tile < 0:
                        continue
                    rows.append({
                        "tile": tile,
                        "ev": value_at(stat.get("exp_score", []), turn),
                        "win_prob": value_at(stat.get("win_prob", []), turn),
                        "tenpai_prob": value_at(stat.get("tenpai_prob", []), turn),
                    })
                rows.sort(key=lambda row: row["ev"], reverse=True)
                results.append({
                    "hand": hand,
                    "config": name,
                    "searched": int(raw["searched"]),
                    "engine_time_ms": float(raw.get("time", 0)) / 1000.0,
                    "wall_time_ms": wall_ms,
                    "profile": raw.get("profile"),
                    "rows": rows,
                })
                print(hand, name, results[-1]["searched"], f"{wall_ms:.1f} ms",
                      flush=True)
    finally:
        engine.close()

    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(json.dumps(results, ensure_ascii=False, indent=2) + "\n",
                           encoding="utf-8")


if __name__ == "__main__":
    main()
